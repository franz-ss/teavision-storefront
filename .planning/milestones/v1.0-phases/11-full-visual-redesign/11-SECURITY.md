...
| T-11-03 | Tampering | `retired one-off asset extraction script` | mitigate | Script writes to fixed `public/images/${id}.png` paths; `newsletter-label.png`, `catalogue-cup.png`, `newsletter-teapot.png`, and `business-handshake.png` all have PNG magic bytes. | closed |
...
| T-11-16 | Elevation (XSS) | legacy curved-label component SVG port | mitigate | `retired homepage curved-label component` uses static JSX SVG/textPath and `next/image`; no `dangerouslySetInnerHTML`. | closed |
