...
1. **[MEDIUM] LCP / image weight** — the full-bleed `organic_tea…png` becomes the LCP element. `eager` + `fetchPriority="high"` are set, but: confirm the Shopify CDN size request is sensible (the plan uses `getSizedShopifyImageUrl(..., 1920)`), and consider `quality`/format params. *Action:* add a verification note to eyeball the transferred image size and LCP in the human-verify checkpoint.
2. **[MEDIUM] Text contrast over image** — white `text-on-brand` legibility depends on the `bg-inverse/50` scrim against a potentially light/busy banner. *Action:* verify WCAG AA contrast on the actual asset during the checkpoint; if marginal, deepen the scrim (e.g. `bg-inverse/60`) or add a gradient — still token-based.
