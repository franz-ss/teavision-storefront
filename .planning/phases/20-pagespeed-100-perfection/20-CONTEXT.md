...
- **D-16:** **LEAN SCOPE (owner directive, 2026-06-30, supersedes the broad plan).** Phase 20 is narrowed to resolve EXACTLY the homepage `/` issues in the PSI screenshots (`20-PSI-EVIDENCE.md`) — nothing more. Lean, minimal, production-ready, architecture-preserving, no regressions/side effects. In scope: (1) hero `fetchPriority="high"`, (2) AVIF output (`images.formats`), (3) lazy-load the client Sentry SDK, (4) `experimental.inlineCss` only if measured-beneficial. Active plan: `20-01-PLAN.md` only. DEFERRED (not deleted — moved to `deferred/20-02..05-PLAN.md`, recoverable from commit e0e42881): the full PSI harness, `/account` CLS, Accessibility/Best-Practices/SEO→100, font cleanup, collection-preload/PDP/caching tuning, the tags-live third-party ceiling, and superseding PERF-01. The "100/100 across all four categories" ambition is paused in favour of the targeted homepage Performance fixes. This narrows (does not replace) the broader requirement set: PSI-01/02/04/09/10/11/12/13/14/15 are deferred; PSI-03/05/06/07/08 are addressed by the lean plan.
