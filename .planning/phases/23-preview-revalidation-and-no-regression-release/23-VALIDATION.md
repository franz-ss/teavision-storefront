...
| **Quick run command**  | `pnpm test:unit -- src/lib/sanity/client.test.ts src/lib/sanity/home-page.test.ts src/lib/sanity/queries/home-page.test.ts src/app/(storefront)/page.test.tsx` |
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 23-01-01 | 01 | 1 | PREVIEW-01, PREVIEW-02 | T-23-01/T-23-02/T-23-03 | Draft helpers require preview token, use drafts perspective, and never cache under published tags. | unit | `pnpm test:unit -- src/lib/sanity/home-page.test.ts` | exists | passed in task commit `e71e2163`; final focused unit sweep also passed; 2026-07-03 validation follow-up added direct Sanity client boundary coverage in `d43608de` |
...
- [x] `src/lib/sanity/client.test.ts` - published and draft Sanity client configuration coverage.
...
| `pnpm exec vitest run --environment node src/lib/sanity/client.test.ts`                                             | Pass            | 1 file / 4 tests. Validation follow-up proves the published client uses published perspective and the draft client requires the read token, uses drafts perspective, disables CDN, disables stega, and dispatches `sanityDraftFetch()` through the draft client. |
| `pnpm test:unit -- src/lib/sanity/client.test.ts`                                                                   | Pass            | 70 files / 301 tests. The package script forwarded the requested file after Vitest's own `--`, so the unit suite fanned out broadly while including the new Sanity client boundary test.                                                                         |
| `pnpm exec eslint src/lib/sanity/client.test.ts --quiet`                                                            | Pass            | Narrow lint check for the added validation test file.                                                                                                                                                                                                            |
...
| `pnpm typecheck`                                                                                                    | Pass            | Initially caught missing draft observability event names and `NextRequest` test typing; fixed in `c0852302`, then passed; re-run after the client boundary follow-up also passed.                                                                                |
...
## Validation Audit 2026-07-03 - Client Boundary Follow-Up
...
| Gap                                                                                                                                                                                                                                   | Resolution                                                                                                                                                                  | Verification                                                                                                                                                                                                                                               |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getDraftHomepage()` coverage mocked `sanityDraftFetch()`, so the actual `src/lib/sanity/client.ts` draft-client factory was not directly proving token-backed `perspective: "drafts"`, `useCdn: false`, and `stega: false` behavior. | Added `src/lib/sanity/client.test.ts` coverage for `getSanityClient()`, `getSanityDraftClient()`, missing draft token failure, and separate published/draft fetch dispatch in `d43608de`. | `pnpm exec vitest run --environment node src/lib/sanity/client.test.ts` passed with 1 file / 4 tests; `pnpm test:unit -- src/lib/sanity/client.test.ts` passed with 70 files / 301 tests; `pnpm exec eslint src/lib/sanity/client.test.ts --quiet` passed. |

Result: `nyquist_compliant` remains `true`; the direct Sanity client boundary now has automated coverage, and owner-gated PSI/Sanity publish checks remain manual-only release gates.
